...
- **Startup exports consumed by hooks**: identical to the dev side — see [`dev-agent-flow.md` § Startup exports consumed by hooks](dev-agent-flow.md#startup-exports-consumed-by-hooks). `BASE_BRANCH`, plus `AUTONOMOUS_PROJECT_DIR` and (when set) `PUSH_ALLOWED_REMOTE_URLS` for Layer-1 trunk protection ([INV-148](invariants.md#inv-148-layer-1-trunk-protection-compares-push-destinations-against-an-exported-project-anchor-and-fails-closed-when-either-is-unresolvable)). The review side exports them too because a review session can itself push — a conflict rebase — and must be judged by the same destination comparison as a dev session.
...
- **Failure actionability is explicit and lane-local ([INV-150](invariants.md#inv-150-a-pre-fan-out-e2e-failure-may-be-classified-only-by-an-optional-wrapper-owned-private-lane-sidecar-while-inv-92s-verdict-trailer-remains-the-single-durable-actionability-authority)).** The wrapper exports `E2E_FAILURE_CLASSIFICATION_FILE=$lane_dir/e2e-failure-classification` to both command and browser lanes. A non-zero lane may atomically install exactly `dev-actionable=true` or `dev-actionable=false`. Absence preserves legacy `true`; any present non-regular, symlink, oversized, unreadable, or malformed object fails closed to `false`. Exit codes, logs, report prose, and LLM output are never classifiers. Before `pending-dev`, the wrapper requires the neutral `result=e2e-failed` disposition and the matching INV-92 trailer; the disposition intentionally carries no duplicate actionability.
  INV-122 may preempt an actionable repeat only after machine-authored current-HEAD INV-85 evidence proves the bounded DEV correction was already consumed; historical breaker count alone is insufficient, while non-actionable failures always reach the required INV-150/INV-92 durable handoff.
...
```
 ...
 (cause smoke-config-   UNCHANGED; full fan-out runs;                         │
 error); RESULT_PARSED  resulting all-unavailable                             │
 =true (crash trap not  counts toward INV-144                                 │
 overriding); stays                                                          │
 `reviewing`; exit 1                                                         │
```
...
- **(b)** after the poll window, if a codex member that **exited rc 0** (the SOLE gate) produced stdout but **no** self-posted verdict landed, the wrapper classifies the stdout (`_codex_review_classify_stdout`: any `[P0]`-`[P3]` → `fail`, else `pass` — the gate the manual `/codex review` skill uses), composes the canonical body (`_codex_review_compose_body`), and posts it via `post-verdict.sh` as agent `codex`, then re-polls. EVERY completed (rc 0) review posts exactly one verdict — an empty capture (→ default PASS body) and a capture whose text merely **mentions** the stream-error phrases (no `[P0]`-`[P3]`) both post PASS. There is **no stream-error skip on the rc-0 path** (#218 finding 5): a genuine stream failure exits non-zero (filtered by the rc-0 gate → `unavailable` + the stream-error drop-reason), and `_codex_review_has_stream_error` is a broad substring scan that would false-positive on review TEXT about a stream error — so it gates only the drop-reason path, never the fallback. **The tag scan runs over the structurally-stripped final response, not the raw whole capture, whenever the capture validates as a genuine turn-marker review ([INV-132], issue #481 review round 4)** — see [Pre-aggregation severity filter](#pre-aggregation-severity-filter-issue-449-r1) below for why scanning the raw capture is unsafe;
...
**Review round 2 finding — the fallback route was reachable in name only.** A genuine `codex review` turn-marker capture, BY CONSTRUCTION, has the wrapper's own prompt echoed verbatim as the `user` turn's content — so its leading region satisfies [INV-73]'s malformed-echo signals 1/2 exactly like a pure prompt-echo/startup-trace does. `_codex_review_classify_stdout` runs the malformed check FIRST, so every turn-marker capture — including one whose final `codex` turn carries a genuine, fully-formed review — was classified `malformed` before the `codex-stdout-fallback` tagging above could ever run, leaving codex unresolved for the terminal sweep (`unavailable`) instead of reaching the severity call site. The fix adds a signal 0 to `_codex_review_stdout_is_malformed`, checked before signals 1-3, that reuses `_codex_review_strip_prompt_echo`'s own boundary parse: if a validated header → `user` marker → `codex` marker structure exists AND real text survives strictly after the last `codex` marker, the capture is a completed turn-marker review and is NEVER malformed, regardless of what its own header/echo looks like. A turn-marker capture with markers but no post-marker text (captured mid-turn) does not satisfy signal 0 and correctly falls through to signals 1-3 unchanged.

**Review round 3 finding — an UN-FENCED inline `codex` word, with no blank line before it, could still win the last-marker search.** Fencing alone (round 1's fix) does not reject a bare `codex` word flowing directly out of the end of a prose sentence in quoted tool/reviewed-file output (e.g. "Tool output follows:" then a literal `codex` line) — that shape is column-0, exact-word, and unfenced, so it satisfied the pre-round-3 discipline and, being the LAST such candidate, discarded every real finding (including a `[P1]`) that preceded it. Every genuine turn marker, in every known capture shape, is its own paragraph — preceded by a blank line — because the CLI always closes out the prior turn's text before opening a new one. The fix adds that requirement to the marker search: a candidate line is accepted only when the immediately preceding line (tracked across the same fence-toggle state) is blank. This narrows the existing structural discipline; it introduces no new false negatives against any known genuine capture shape.

**Review round 4 finding — `_codex_review_classify_stdout` itself still scanned the RAW whole capture, independently poisoning a clean review.** Rounds 1-3 fixed the severity-FILTER's input selection and the malformed-gate reachability, but `_codex_review_classify_stdout` — the fallback classifier that decides `pass`/`fail` in the first place, BEFORE the severity filter ever runs — still ran its `[P0]`-`[P3]` substring scan over the ENTIRE raw capture once a turn-marker review is admitted past the malformed gate. A genuine turn-marker capture echoes the wrapper's OWN prompt verbatim as the `user` turn's content, and that prompt's severity-tagging instruction block (`_review_severity_prompt_block`) literally quotes `` `[P0]` ``…`` `[P3]` `` as backtick-fenced markers defining the vocabulary — a substring match with no structural awareness, so it fired on that quoted instruction text exactly like a real finding tag. The result: a codex review whose final response carries **no findings at all** still classified `fail`, and the severity filter downstream could never rescue it — the (correctly stripped) clean final response scores `none` under `_review_extract_highest_severity` (no tag present at all is exactly the fail-safe case), and `none` ALWAYS blocks (`shouldBlockFinding`'s fail-safe default) — so a clean stdout-fallback review would block indefinitely, at every round, regardless of the severity ratchet. The fix reuses the SAME strip helper the malformed gate's signal 0 already validated the capture's structure with: `_codex_review_classify_stdout` now scans `_codex_review_strip_prompt_echo`'s output when stripping actually changed the text (a genuine turn-marker capture with real content after the last `codex` marker) — i.e. it scans exactly the same final-response text the severity filter will later score — and falls back to scanning the whole capture unchanged when there is no turn-marker structure to strip (the legacy free-form shape, byte-identical to pre-round-4 behavior for that shape).

On demotion, `AGENT_VERDICT_BODIES[i]` is ALSO re-rendered: the original body's `Review findings:` prefix and `[BLOCKING]` markers are replaced with an explicit non-blocking note (keeping the underlying finding text for transparency) — otherwise the demoted body would flow into `LATEST_COMMENT` unchanged and get posted as a self-contradictory `Review PASSED - Review findings: ... [BLOCKING] ...` comment that could also make a LATER dev-resume session misread an already-passed review as still having outstanding blocking feedback (the dev-resume prompt scans for the literal `Review findings:`/`[BLOCKING]` substrings).
...
Note that `_codex_review_classify_stdout` (the codex stdout→verdict fallback classifier, [INV-62]) itself now flags `fail` on ANY severity tag (`[P0]`-`[P3]`), not just `[P1]` as before #449 — the round-aware demotion decision happens strictly LATER, in this filter, not in that classifier. As of the round-4 fix above, that scan also runs over the SAME structurally-stripped final-response text as this filter (not the raw whole capture) whenever the capture validates as a genuine turn-marker review — so the classifier's `pass`/`fail` decision and this filter's severity extraction are now scored from IDENTICAL text on that route.
...
The aggregate maps onto the existing `PASSED_VERDICT` / `LATEST_COMMENT` / `AGENT_EXIT` variables. `all-unavailable` sets `LATEST_COMMENT=""` and falls back to the single-agent FAIL path, preserving the legacy `AGENT_EXIT` distinction so N=1 remains compatible: `AGENT_EXIT=1` when any agent's CLI actually crashed (rc ≠ 0) → crash-fallback comment + `failed-non-substantive other`; `AGENT_EXIT=0` when every agent exited cleanly but posted no verdict → no crash comment + `failed-substantive`. On *partial* unavailability the wrapper posts one human-visible summary comment (dropped vs. deciding agents) and logs a WARN, then decides on the deciding agents.

**Unavailable-cap escalation breaker ([INV-144](invariants.md#inv-144-consecutive-all-unavailable-review-aggregates-are-bounded-by-an-independent-durable-breaker-that-transitions-reviewing---stalled-and-posts-one-reasonreview-unavailable-cap-report), #525).** Before `all-unavailable` reaches the ordinary pending-dev route, the wrapper advances the independent `dispatcher-review-unavailable-breaker` marker. `REVIEW_UNAVAILABLE_CAP` defaults to 3 (`0` disables unavailable-round increments/tripping; `1` trips immediately; malformed warns and falls back). Rounds are head-agnostic and every round persists, including the first. Every decided `pass` or `fail` aggregate posts this breaker's explicit `round=0` reset marker, including `head=unknown` and while the cap is disabled, so neither a missing Reviewed-HEAD post nor an ambiguous verdict trailer can preserve a stale streak after re-enabling. Marker reads request exact self-author classification (`ITP_REQUIRE_SELF_AUTHOR=1`), accept only `authorKind=self`, and require the marker's `issue=` field to equal the active issue; unrelated bots cannot manipulate the streak. At the threshold the wrapper respects an existing sibling `stalled` state, otherwise transitions `reviewing -> stalled`, sets `RESULT_PARSED=true`, then persists the threshold marker and an independent `round=0` re-arm marker before attempting one `reason=review-unavailable-cap` report. Cutoff scanning also reads the provider-neutral latest `stalled` removal event, so operator re-arm restarts at one even if every post after the prior stalled transition failed. The report includes per-agent `_dropped_reasons` and separate INV-64 `_smoke_reasons`; partial-smoke reasons are appended to that shared in-memory collector before fleet shrink, and a signal-free drop is reported as `no reason token recorded`, never guessed. INV-127 remains unchanged and never counts `all-unavailable`.
...
- **Why `pending-review`, not `pending-dev`, on the wait branch.** There is no new commit for a dev agent to make — the code is fine, the wrapper is just waiting for an *external* bot. Routing to `pending-dev` would strand the issue: the [#106 stale-verdict dispatcher guard](state-machine.md) keeps `pending-dev` + open-PR + no-new-commits parked. `pending-review` re-queues a clean re-review on the next tick, by which time `drain_agent_bot_triggers` has fired and the bot's review is present. This is the only `reviewing → pending-review` producer in the mandatory-bot-review gate (the CI-rollup gate, § below, adds two more of its own for the same no-new-commits reason: head-changed and the below-cap CI wait).
...
1. **Per-finding (agent, into the artifact)**: for each blocking finding, set `actionable_by_dev_agent` / `requires_human` / `requires_privileged_token` / `blocking_for_merge` (boolean) and `recommended_next_owner` (`dev_agent`|`human`|`maintainer`). The rule (also re-validated wrapper-side from the schema-checked artifact, TOCTOU-safe): a finding on a **protected path** (`review_path_is_protected` over `REVIEW_PROTECTED_PATHS`) ⇒ `actionable_by_dev_agent=false`, `requires_human=true`, `recommended_next_owner=maintainer` (and `requires_privileged_token=true` only for a `.github/workflows/**` edit when `agent_token_has_workflow_scope` is false); else `actionable_by_dev_agent=true`, `recommended_next_owner=dev_agent`. These two probes live in `lib-review-classify.sh` and are pure config-var reads (the protected list + `AGENT_TOKEN_PERMISSIONS`) — NO GitHub API call, NO sidecar. The **classification rule sent to the agent in the prompt is generated from the same `$REVIEW_PROTECTED_PATHS` value** (`review_protected_paths_prompt_rule`, also in `lib-review-classify.sh`), so an operator override is reflected verbatim in the prompt — the prompt and the wrapper-side matcher cannot diverge (issue #301), and the `requires_privileged_token` guidance for a `.github/workflows/` edit reflects the SAME `agent_token_has_workflow_scope`/`GH_AUTH_MODE`/`CODE_HOST` check the default derivation uses (never a hardcoded scope claim, [INV-136]). The default list is applied only when the var is **unset** (`${VAR-default}`); an explicit `REVIEW_PROTECTED_PATHS=""` is honored as "no protected paths" (every finding dev-actionable, the pre-#298 behavior), and the prompt then advertises that there are no protected paths. **[INV-136] (#488) — the default itself is capability-aware**: `.github/workflows/** CODEOWNERS .github/CODEOWNERS` unless `CODE_HOST == "github"` (or unset) AND `GH_AUTH_MODE=app` AND `agent_token_has_workflow_scope` (a pure `AGENT_TOKEN_PERMISSIONS` config-var read) proves the minted agent token carries the `workflows` scope, in which case the default omits `.github/workflows/**` — a token/PAT-mode install, a non-`github` `CODE_HOST` (e.g. `gitlab`, even with a leftover `GH_AUTH_MODE=app` conf fragment — GitLab mints no scoped GitHub App token, so those vars prove nothing there), or any App-mode install where the scope is absent/unknowable, keeps the full conservative default (fail-closed). An EXPLICIT `REVIEW_PROTECTED_PATHS` value (including `""`) is NEVER rewritten by this check, in either direction — the capability probe only ever runs as the unset-branch default.
...
3. **Matched-pattern diagnostics ([INV-136] D4)**: from the SAME live `_art_json`, the wrapper also stashes each FAILing agent's sorted/unique matched `REVIEW_PROTECTED_PATHS` pattern(s) (`review_classify_artifact_matched_patterns`) into a surviving `AGENT_MATCHED_PATTERNS[]` global. The wrapper collects the cross-agent `_AGG_MATCHED_PATTERNS` UNCONDITIONALLY — INDEPENDENT of the aggregate `dev-actionable` bit — and posts a dedicated findings comment whenever ANY pattern was recorded, naming the matched pattern(s) + the `REVIEW_PROTECTED_PATHS` conf lever, carrying a machine-readable, **head-bound** `<!-- inv92-matched-patterns: head=<sha|unknown> <patterns> -->` marker (the `head=` field is `${PR_HEAD_SHA:-unknown}` — the round's reviewed head, never an empty field). This matters for a MIXED FAIL (one blocking finding forced non-actionable by a protected-path match, plus a second, ordinary finding that IS actionable): the aggregate is `true` there (the OR over blocking findings), but D4's own contract — name the pattern whenever the derivation forced ≥1 finding non-actionable — still applies, so gating the collection on the aggregate would silently drop the diagnostic for exactly this case. The comment's lead sentence still branches on the aggregate so the wording never overclaims total non-actionability in the mixed case. The dispatcher's own stall notice (below) only ever sees the coarse trailer bit, never the per-finding path, so this marker is the sole channel that lets the stall notice name WHICH pattern forced an escalation — but the marker being posted on a mixed (aggregate=true) FAIL is inert for the dispatcher, which only reads it from the `dev-actionable=false` branches.

The dispatcher (`handle_completed_session_routing` Branch B′) routes a `failed-substantive` + `dev-actionable=false` to escalation (an idempotent per-HEAD `@owner` notice with `reason=non_actionable_finding`, then `mark_stalled`) instead of dev-resume — see [`dispatcher-flow.md`](dispatcher-flow.md) and [INV-92](invariants.md#inv-92-a-review-blocking-finding-the-dev-agent-provably-cannot-act-on-protected-path--missing-token-scope-is-not-routed-to-dev-resume--the-wrapper-classifies-each-findings-actionability-and-the-dispatcher-escalates-a-non-actionable-verdict-to-stalled). An absent token (legacy / artifact malformed) defaults `true` ⇒ today's dev-new path (zero regression). When the review wrapper's `inv92-matched-patterns:` marker is present on the issue **and its `head=` field matches the head this verdict was reviewed against**, `lib-dispatch.sh::_inv92_matched_patterns <issue_num> <reviewed_head>` reads it and the stall notice names the matched pattern(s) + the conf lever; when absent, or when a marker exists only for a DIFFERENT (stale, earlier-round) head, the notice falls back to the pre-[INV-136] generic wording (no new failure mode, and no misattribution of a stale pattern to an unrelated later failure — the head-binding fix from PR #498 round 2). See [INV-136](invariants.md#inv-136-the-default-review_protected_paths-is-capability-aware-in-app-mode--githubworkflows-is-omitted-from-the-built-in-default-iff-gh_auth_modeapp-and-the-minted-agent-token-provably-carries-the-workflows-scope-every-other-case-token-mode-gitlab-scope-absent-malformed-conf-keeps-the-conservative-default-and-an-explicit-operator-value-is-never-rewritten-in-either-direction) for the full default matrix.
...
flowchart TD
    ...
    verdict_trailer --> to_dev["_teardown_call terminal_intent_cleanup_transition<br/>live intent: reviewing -> stalled -> consume<br/>none: original reviewing -> pending-dev<br/>(bounded 60s)"]
    to_dev --> auth_done
```
...
**Bounded network calls** ([Lane-GC PR-3, INV-115](invariants.md#inv-115-cleanup-reaps-registry-recorded-process-groups-before-any-network-call-under-a-shared-reaplock-with-every-network-call-wall-clock-bounded)): every GitHub-API call in this trap — `drain_agent_bot_triggers`, `get_gh_app_token`, `itp_post_comment`, `emit_verdict_trailer`, and the `itp_list_comments` / `itp_read_task` / `itp_transition_state` operations behind terminal-intent cleanup — routes through the same `_teardown_call`/`_bounded_call` (`lib-lane.sh`) 60-second bound the dev wrapper uses, so a hung `gh` invocation in the crash path can never park this trap indefinitely.
