...
> **Same-HEAD mergeability freshness ([INV-149], #545):** the
> `failed-non-substantive` marker-present clause in the same-HEAD row has one
> provider-state exception. When the historical cause is
> `mergeable-unknown`, the marker alone is not terminal evidence. The helper
> runs the same bounded mergeability poll used by review preflight between two
> provider snapshots of the same open HEAD. Current `MERGEABLE` or
> `CONFLICTING` requeues `pending-review` so the ordinary preflight reaches
> fan-out or the canonical conflict/rebase route. Only a poll whose reads all
> succeed and remain non-decisive can corroborate persistent `UNKNOWN` and
> retain the existing stall; one unsettled read cannot.
>
> Each freshness attempt first writes an idempotent per-ordinal intent
> reservation scoped to the dev session and full HEAD. Reservations, not the
> post-transition completion marker, count toward `REVIEW_RETRY_LIMIT`; an
> ambiguous label response or failed completion write therefore cannot escape
> the cap. Reservation/completion pairs and duplicate posts with the same
> session + HEAD + ordinal count once. The completed-session router reads this
> same logical budget, so completion becoming provable cannot open another
> retry lane. Intent-count/read/write, malformed timeline, and undecided
> provider-read failures return operational defer (`3`) so INV-128 can bound a
> stable outage. Both completed-session entry points treat `3` as a per-issue
> continue without `JUST_DISPATCHED`; other nested errors retain their hard
> propagation. Label failures also return `3` below the cap, while their durable
> reservations make repeated failures converge without aborting the whole
> project tick.
...
This is the Step 4 mirror of Step 5b's "DEAD + in-progress + PR exists"
branch — it covers the case where the cleanup trap got there first and the
issue is already on `pending-dev` when the next tick arrives. Matching routing
evidence prevents both the prior-review-FAILED loop and the INV-147
pre-fan-out conflict loop from sending an unchanged HEAD back to E2E.
...
- **residual** (a wrapper IS still alive per `may_stall_now`, for either precondition above; a concurrent tick holds the `dev-new` dispatch marker, [INV-108]; the helper's comment-read preflight failed; or [INV-149]'s bounded provider poll / intent / label write failed) → the `stale-verdict:<sha>` park stays. These are genuinely transient. INV-149 operational failures return defer (`3`) so the tick omits `JUST_DISPATCHED` and INV-128 can bound a persistent outage. This is narrower than the pre-[INV-125] park: a marker-present budget-exhausted case with usable current evidence (the self-heal/crash-recovery `dev-new` already ran for this HEAD, or the shared non-substantive re-review budget is spent) is handled inside the helper instead of falling here. `is_session_completed` needs a readable `{"type":"result"}` line from the dev wrapper's per-issue log, which the **claude** dev adapter produces via `--output-format json`; for non-claude dev CLIs it returns false by design (see its per-CLI scope) — but as of [INV-125] that no longer means an unconditional park, only that the `crashed-session` cause of the shared helper engages instead of `self-heal`. The log read itself is backend-aware ([INV-101](invariants.md#inv-101-is_session_completed-is-authoritative-under-all-execution-backends--terminal-state-detection-consults-a-backend-specific-log-probe-mirroring-inv-30s-pid_alive-shape), #356): under `local` it's a direct `/tmp/agent-${PROJECT_ID}-issue-N.log` read; under `EXECUTION_BACKEND=remote-aws-ssm` it's the same read run on the execution host via `session-log-probe-remote-aws-ssm.sh` over SSM (mirroring [INV-30]'s `pid_alive` shape) — a remote transport fault collapses to the same "not completed" path, never a fabricated completion.
...
The shared helper closes that gap with the SAME guard rails the rest of the INV-35/INV-85 machinery uses, **plus a verdict-classification step that runs before any of them**: `classify_recent_review_verdict` (called with an empty `session_end_iso` — there is no session to anchor to, and every ISO-8601 timestamp sorts greater than `""`, so the classifier still returns the newest qualifying trailer) is consulted first so a `failed-non-substantive` (bot/CI/transport hiccup) or `dev-actionable=false` ([INV-92]) verdict never burns a `dev-new` it can't use — those route to `pending-review` / `mark_stalled` instead. The `dev-actionable=false` marker is per-cause (`self-heal-non-actionable:<head>` / `crashed-session-non-actionable:<head>` — it never dispatches, so there's no shared budget to protect); the `failed-non-substantive` re-review marker is **shared** across both causes (`self-heal-non-substantive:<head>`, one namespace — a re-review costs the same regardless of which cause requested it). On a repeat `mergeable-unknown`, INV-149 refreshes the provider before that marker may become terminal. Only a `failed-substantive` (dev-actionable=true) or unclassifiable (`none`) verdict reaches the `dev-new` dispatch, which keeps its existing guard rails: `may_stall_now` liveness (never race a wrapper that might still post its own id), `acquire_dispatch_marker`/[INV-108] controller-side dedup (this is a THIRD dev-new entry point, alongside `dispatcher-tick.sh`'s own sites and [INV-98]'s delegation), and a **shared** per-HEAD dev-new budget check across both `self-heal-lost-session:<head>` AND `crashed-session-retry:<head>` — either marker present means this HEAD already consumed its one self-heal/crash-recovery `dev-new` with no progress, regardless of which cause is asking now. A fresh dispatch posts only its OWN cause-specific marker (never both).

**Part 2 ([INV-125]): closing the counting hole.** Every marker-present / budget-exhausted arm above is terminal once its evidence is current. The [INV-149] exception first refreshes historical `mergeable-unknown`: known current states re-enter review under the existing per-session/per-HEAD `REVIEW_RETRY_LIMIT`; an undecided poll containing a read failure defers under INV-128; and only a fully successful bounded poll that remains `UNKNOWN` reaches `mark_stalled`. Other non-substantive causes and the shared dev-new budget still call `mark_stalled` directly. A park posts only idempotent, liveness-registered notices, which `count_retries` never counts (no dispatch → no countable comment), so a marker-present fall-through with usable evidence would freeze `count_retries` forever with `MAX_RETRIES` never able to trip.
...
| `failed-substantive`, unchanged reviewed full HEAD, and the current dev attempt posted the strict `dev-conflict-context-read-failed` marker | `mark_stalled` immediately with a context-specific notice; dispatch no further `dev-new`. For completed sessions, a strict current-HEAD `conflict-rebase` disposition also supplies the substantive route when the normal classifier returns `none`, because the required preflight verdict predates the dev session. `_dev_conflict_context_failed_current_attempt` accepts only a whole-body issue/HEAD marker newer than the latest trusted `dev-new`/`dev-resume` token, using normalized comment IDs to break timestamp ties. A marker from an earlier attempt is ignored. This direct bound is required because the wrapper session report changes INV-128's generic fingerprint. |
| `failed-substantive`, **no-progress / bot-unfixable** ([INV-85](invariants.md#inv-85-the-completed-session-failed-substantive-route-is-bounded-to-one-dev-new-per-unchanged-head-a-no-progress-or-bot-unfixable-finding-escalates-to-stalled-never-loops)) | **Escalate, do NOT dispatch dev-new.** Both forms require `current PR headRefOid == last Reviewed HEAD: trailer` (HEAD has NOT advanced): EITHER (a) `dev_report_bot_unfixable` — reported *by the dev agent in the current attempt* (a DEV-AUTHORED comment, author resolved from the `Agent Session Report (Dev)` comment, so a reviewer/maintainer/owner/human quoting the signal doesn't count; scan lower-bounded at the current attempt's `dispatcher-token … mode=dev-*` comment — so a prior same-HEAD attempt's signal expires at the next dispatch). As of #511 the detector checks THREE things in order: (1) a structured `<!-- dev-blocked-403: head=<sha> -->` marker with `head=` matching the current PR head — the PRIMARY signal, suppresses (2)/(3) whenever any such marker exists anywhere in the window (even a non-matching one); (2) a success-comment veto (legacy path only, no marker in window) — if THIS attempt's `Agent Session Report (Dev)` shows `Exit code: 0` AND HEAD demonstrably moved during the attempt (a pre-attempt `Reviewed HEAD:` trailer differs from the current head), any legacy 403 match is ignored (a session that moved the head was not blocked — closes the false-positive where a successful session's incidental 403 mention about an unrelated optional action tripped this branch); (3) the legacy free-text `403 Resource not accessible by integration` + PR-metadata-context substring scan, byte-identical to pre-#511, reached only when neither (1) nor (2) applies. OR (b) a `no-progress-substantive-attempt:<head>` marker already exists (a prior `dev-new` ran for this HEAD and produced no commit). Post a one-time `no-progress-substantive:<head>` notice with @owner mention, then `mark_stalled` (NOT `--at-cap` — review-no-progress state, not retry-budget-exhausted). Per-issue log is NOT truncated. |
...
> **Two entry points into this router** ([INV-98](invariants.md#inv-98-the-step-4a5-same-head-pr-exists-park-is-not-terminal--a-completed-session-delegates-to-the-inv-35-router-only-the-residual-cases-park), #351). `handle_completed_session_routing` is reached from EITHER (1) the tick's Step 4b.5 terminal-state gate (the historical path — reached only when Step 4a.5 found **no** PR), OR (2) [Step 4a.5](#step-4a5-pr-exists-short-circuit-99-bug-3-106)'s same-HEAD branch **delegating** when a PR exists and the session is `completed`. Path (2) is what makes this table reachable after a review FAIL or pre-fan-out disposition at all — either leaves a PR, so Step 4a.5 must delegate instead of taking the first-review shortcut. The delegating caller passes its matched strict routing HEAD as an optional fourth argument. The router re-reads the current PR HEAD once before acting and reuses that snapshot; a changed or unreadable HEAD returns to `pending-review` without dispatching dev or writing an INV-85 attempt marker for the new HEAD. When the HEAD remains pinned, that comparison lets one failed conflict-rebase attempt reach INV-85 `stalled` instead of dispatching a second `dev-new`. Historical callers that omit the fourth argument continue to use the original INV-04 `last_reviewed_head` consumer.
...
**#511 extension — structured marker + success-comment veto, on top of the same scoping above.** Observed live (issue #485 / a companion PR, 2026-07-16): a dev session that FIXED every review finding, pushed a new commit, and hit an unrelated 403 trying to retrigger a flaked third-party CI action (an optional courtesy step it separately reported in its completion comment) tripped the free-text substring scan — the next round's genuinely dev-actionable finding was stalled instead of dispatched, because the scan could not distinguish "my 403 blocks this finding" from "I hit a 403 on an optional side-action while otherwise succeeding." `dev_report_bot_unfixable` now resolves in three ordered steps, still inside the SAME author-allow-list / per-attempt-token-window / review-comment-exclusion scoping documented above:

1. **Structured marker (primary signal)**: a dev-authored, in-window comment carrying `<!-- dev-blocked-403: head=<sha> -->`. The dev-side prompt (`autonomous-dev.sh`'s `DEV_BLOCKED_403_MARKER_BLOCK`, see [`dev-agent-flow.md`](dev-agent-flow.md)) instructs the agent to emit this marker ONLY when a review finding is genuinely blocked by the token's scope — never for an incidental/optional-action 403. If ANY such marker exists anywhere in the window (matching head or not), the legacy substring path (step 3) is skipped entirely: the dev agent has become marker-aware for this attempt, so free-text commentary is no longer a signal at all. Unfixable fires only when the marker's `head=` field exactly equals the caller-supplied current PR head; a stale-head marker (superseded by a later push) returns NOT-unfixable.
2. **Success-comment veto (defense in depth, legacy path only)**: reached only when no marker exists anywhere in the window. If, within the SAME attempt, the dev agent's `Agent Session Report (Dev)` shows `Exit code: 0` AND HEAD demonstrably moved during the attempt — the most recent `Reviewed HEAD:` trailer that PREDATES the current dispatch token differs from the caller-supplied current head — any legacy substring match in this attempt is ignored. Both conditions are required: exit-0 alone doesn't prove progress (a no-op session can exit 0 without pushing), and a moved head alone doesn't prove non-blockage. Requiring an actual pre-attempt `Reviewed HEAD:` trailer (not merely a non-empty current head) is a deliberate fail-safe — no trailer found means no evidence of movement, so the veto does NOT fire, preserving the original all-or-nothing legacy behavior (this is what keeps the #274 round-4 same-HEAD-completion-403 regression correctly classified unfixable).
3. **Legacy substring fallback (byte-identical to pre-#511)**: reached only when no marker exists anywhere in the window AND the success-veto did not fire. Same scan as before this issue — a legacy deployment (dev agent not yet marker-aware) or a genuinely blocked session with no exit-0-and-head-moved evidence degrades to exactly today's behavior.

All three steps stay fail-open toward NOT-unfixable on a `gh`/jq transport error, matching the pre-#511 contract. See [INV-85](invariants.md#inv-85-the-completed-session-failed-substantive-route-is-bounded-to-one-dev-new-per-unchanged-head-a-no-progress-or-bot-unfixable-finding-escalates-to-stalled-never-loops) for the normative rule text.

This branch composes with the auto-merge-failure recovery from #146: every dev
mode prepends "rebase onto main" only for an `Auto-merge failed:` PR comment
whose exact final line is either the current-HEAD INV-33
`auto-merge-failure` marker or INV-147 `auto-merge-conflict` marker.
Prefix-only, quoted, stale, malformed, terminal-newline, and wrong-issue
comments do not route. Thus a substantive failure caused by an
auto-merge-blocking rebase still receives the mandatory guidance without
letting historical or human-authored lookalikes trigger it.
...
| **Idle** | `now - PR.updatedAt > 300s` (strict `-gt`, [INV-10](invariants.md#inv-10-5-minute-idle-gate-before-sigterm)) | Recent activity; agent may be cleaning up. Necessary but — since [INV-137] — no longer sufficient on its own. |
...
When all gates hold, the **final recheck + signal** differs by backend ([INV-137]):

- **Local**: re-read `issue-<N>.pid` (must equal the pid the STALE snapshot captured), re-run `kill -0 $PID` (unchanged from pre-[INV-137]), re-run `dev_progress_snapshot` and require STALE with the SAME `pid`/`run_id` as the initial snapshot. Only then `kill $PID` (SIGTERM, NOT SIGKILL — wrapper trap needs to clean up).
...
No state-machine label edge changed — the transition is still `in-progress → pending-review`, exactly as before [INV-137].
...
- Any agent-progress snapshot classified `UNKNOWN` ([INV-137]) → log WARN naming the reason token, leave issue alone. UNKNOWN never falls back to the idle gate alone — that would resurrect the exact false-SIGTERM bug this invariant closes.
...
- `non_idempotent_comment_count` — total comments MINUS comments matching any of a fixed pattern list covering every known idempotent-notice/marker grammar this repo's breakers post (so a park's own first notice never resets the clock). **[codex review, PR #472, round 8 BLOCKING #2]** Each grammar is matched with a leading wrapper anchor (a backtick code span or an HTML-comment opening immediately before the token) — the pre-round-8 bare alternation was an unanchored substring test that a human comment merely discussing or quoting a token in prose satisfied identically to a genuine wrapped marker/report, corrupting the count in either direction depending on which token was mentioned. **[codex review, PR #472, round 9 BLOCKING #1]** In `GH_AUTH_MODE=app`, a match from an `authorKind=human` comment is now RECLASSIFIED as counted progress rather than excluded as idempotent — closing the inconsistency where this function had no `authorKind` gate at all while the digest/marker reads did. `GH_AUTH_MODE=token` behavior is unchanged.
- `marker_digest` — a sorted digest of which of those grammars are PRESENT (authorKind-gated, SAME wrapper anchor as above), deliberately excluding the watchdog's OWN marker (which would otherwise flip the digest exactly once and then permanently pollute the fingerprint with a component carrying no further information).
- The PR-head component ALSO requires distinguishing "no PR is bound to this issue" from "the PR lookup transiently failed" — see the transport-failure exemption below.

A tick where this fingerprint is unchanged from the prior tick is a **no-op tick**; the running count persists via a `<!-- dispatcher-liveness-watchdog: issue=<N> fingerprint=<hash> count=<n> tier1=<0|1> tripped=<0|1> -->` marker, posted/updated on every evaluated tick (mirrors INV-122's "computed on EVERY round" requirement) as its OWN bare comment — never embedded as the first line of the tier-1/tier-2 report. Read back via `_liveness_prior_marker` (`lib-liveness.sh`) — a CUTOFF-then-scan (mirrors `_review_cap_prior_marker`) using a WHOLE-BODY structural anchor (`^<!-- dispatcher-liveness-watchdog: ... -->[[:space:]]*$` — the ENTIRE comment body must be the marker) that excludes any marker at or before the cutoff, so an operator resuming a stalled issue (removing `stalled` with the fingerprint otherwise unchanged) starts a fresh episode instead of immediately re-tripping.

**[codex review, PR #472, round 8 BLOCKING #1 — cutoff redesign]** cutoff = the latest qualifying marker whose OWN `tripped` field equals `1` (the epoch if none has fired). This replaces three successive rounds (3, 6, 7) of anchoring the cutoff to a SEPARATE, hand-typed heading-text pattern (`_LIVENESS_TIER2_HEADING`) — round 7 tightened `contains()` to `startswith()` after finding a bare mid-comment mention forged the cutoff; round 8 found `startswith()` was STILL forgeable in the default `GH_AUTH_MODE=token` topology, since an unauthenticated comment merely OPENING with the exact heading line satisfied it just as well as a genuine report. Each round patched the same flaw — a second, independently-typed prose pattern carries no authentication of its own — without fixing it. `tripped` is now a FIELD on the already-authenticated, whole-body-anchored marker grammar, so the cutoff computation shares the SAME anchor and the SAME `authorKind` filter as the prior-marker scan it feeds; there is no separate pattern left to drift out of sync or be independently forged. `_LIVENESS_TIER2_HEADING` remains only as the report's operator-facing display heading — no detector reads it. The read is authenticity-gated via `_liveness_strict_author_flag`: `authorKind != "human"` is required ONLY in `GH_AUTH_MODE=app` (mirrors `classify_recent_review_verdict`'s two-part [#389]/[#393] pattern) — token mode relies on the structural anchor alone, with any residual forgery's blast radius bounded by capping the accepted count at `LIVENESS_STALL_TICKS`.

**Exemptions** (never a false-positive no-op): `was_just_dispatched` ([INV-09]); `is_within_grace_period` ([INV-18]); a live wrapper or fresh dispatch marker (`_liveness_wrapper_alive`, composing the SAME `_dispatch_marker_recent` + `pid_alive` primitives [INV-26]'s `may_stall_now` uses — never a new liveness check); an `itp_list_comments` transient failure (skip the tick entirely, fail-toward-defer); **[codex review, PR #472, round 8 BLOCKING #3]** a `fetch_pr_for_issue` transport failure — distinct from "genuinely no PR bound" (rc=0, empty result) — also skips the tick entirely rather than treating the transport failure as an empty head, which would silently reset the counter and mask a genuine park on the tick meant to detect it.

**Two tiers**:

- **Tier 1** (`count >= LIVENESS_NOTICE_TICKS`, default 6): posts the bare marker (`tier1=1`, `tripped=0`) as its own comment, THEN a SEPARATE non-idempotent comment — "no observable progress for N ticks" + the fingerprint components + `@REPO_OWNER`. Latches (`tier1=1` in the marker) so it never re-fires while the fingerprint stays unchanged. The report text is excluded from the fingerprint's own comment-count component (`_LIVENESS_GRAMMARS_JSON`, `lib-liveness.sh`) so it never registers as progress against itself.
- **Tier 2** (`count >= LIVENESS_STALL_TICKS`, default 18): re-checks current labels for `stalled` immediately before acting (mirrors INV-122's already-stalled skip — a specific breaker may have won the race); otherwise `label_swap` to `stalled` runs FIRST atomically (mirrors INV-105/INV-122's TOCTOU fix), THEN posts the bare marker (`tripped=1`) as its own comment, THEN a SEPARATE structured `reason=liveness-timeout` report with the last-known fingerprint, tick counts, and a pointer to the newest session-report/verdict comment.

Both thresholds are read with the same regex-then-fallback-with-stderr-only-warning shape as INV-122's `_gate_breaker_threshold`; `LIVENESS_STALL_TICKS` additionally validates `> LIVENESS_NOTICE_TICKS`. **[codex review, PR #472, round 8 BLOCKING #4]** `run_liveness_watchdog` captures each reader's stderr (via a `mktemp` file, mirroring this file's `ci_is_green` capture-then-relog pattern) and re-emits any WARNING through `log()` instead of discarding it to `/dev/null` — the pre-round-8 code applied the fallback correctly but silently, with no trace in the dispatcher's own log output on a misconfigured run. With a 5-minute tick cadence, the defaults give ~30 min to tier 1 and ~90 min to tier 2 — versus the ~23h and 21-round incidents this closes.

**[codex review, PR #472, round 9 BLOCKING #2]** Every marker/report post now goes through `_liveness_post_marker`/`_liveness_post_report` (`lib-dispatch.sh`) instead of a bare `itp_post_comment ... || true`: each retries once on failure; `_liveness_post_marker` additionally posts a loud `@REPO_OWNER` operator notice and returns 1 on persistent failure (this marker carries the counter/tier1-latch/`tripped` state — a silently lost write would freeze or mis-cutoff the series). The `tier1` branch skips its dependent report when the marker write persistently fails; the `tier2` branch posts its report regardless, since `label_swap` already committed the `stalled` transition beforehand. Every call site that isn't the `tier1` branch's intentional `_liveness_post_marker ... || return 0` appends `|| true`: this file runs under `set -euo pipefail`, so a bare call to a helper that CAN return 1 would trip `set -e` and abort the whole tick — the exact regression `|| true` originally prevented, reintroduced one level deeper. Pinned by a real `bash -euo pipefail` subshell probe (TC-LIVENESS-075..078) since the ordinary `set +e` test harness can't see this class of bug.
